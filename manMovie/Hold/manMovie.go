...
func calcFrames(filePath string) error {
	...
	m.Scale = 1
	// m.X = mOrig.X / mOrig.Scale
	// m.Y = mOrig.Y / mOrig.Scale
	m.X = mOrig.X
	m.Y = mOrig.Y
	...
	for i := 0; i < Frames; i++ {
		...
		// fmt.Println("calcFrames:", FileOut.Name())
...
func setDirectory(filePath string) error {
	...
	fmt.Println("setDirectory:", filePath, file, ex)
	if ex != ".JPG" && ex != ".jpg" {
		return errors.New("File must be a jpeg type.")
	}
	rootName := file[:len(file)-len(filepath.Ext(file))]
	fmt.Println("setDirectory:", rootName)
